...
class TestDiskMonitorConfigValidation(unittest.TestCase):
    ...
    def test_valid_defaults(self) -> None:
        from core.config import DiskMonitorConfig
        from core.config_validation import _validate_disk_monitor_config
        dm = DiskMonitorConfig()
        _validate_disk_monitor_config(dm)

    def test_zero_threshold_raises(self) -> None:
        from core.config import DiskMonitorConfig
        from core.config_validation import _validate_disk_monitor_config
        dm = DiskMonitorConfig(threshold_gb=0)
        with self.assertRaises(ValueError) as ctx:
            _validate_disk_monitor_config(dm)
        self.assertIn("threshold_gb", str(ctx.exception))

    def test_negative_threshold_raises(self) -> None:
        from core.config import DiskMonitorConfig
        from core.config_validation import _validate_disk_monitor_config
        dm = DiskMonitorConfig(threshold_gb=-10)
        with self.assertRaises(ValueError):
            _validate_disk_monitor_config(dm)

    def test_low_interval_raises(self) -> None:
        from core.config import DiskMonitorConfig
        from core.config_validation import _validate_disk_monitor_config
        dm = DiskMonitorConfig(interval_sec=5)
        with self.assertRaises(ValueError) as ctx:
            _validate_disk_monitor_config(dm)
        self.assertIn("interval_sec", str(ctx.exception))

    def test_top_n_out_of_range_raises(self) -> None:
        from core.config import DiskMonitorConfig
        from core.config_validation import _validate_disk_monitor_config
        dm = DiskMonitorConfig(top_n=0)
        with self.assertRaises(ValueError) as ctx:
            _validate_disk_monitor_config(dm)
        self.assertIn("top_n", str(ctx.exception))

        dm2 = DiskMonitorConfig(top_n=101)
        with self.assertRaises(ValueError):
            _validate_disk_monitor_config(dm2)

    def test_disk_monitor_loaded_from_yaml(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            cfg_path = Path(td) / "orca_auto.yaml"
            cfg_path.write_text(
                json.dumps({
            ...
            cfg = load_config(str(cfg_path))
            self.assertEqual(cfg.disk_monitor.threshold_gb, 100.0)
