...
import chemstack.core.utils.process_tracking as process_tracking


def test_current_process_lock_payload_omits_ticks_when_unavailable() -> None:
    with patch("chemstack.core.utils.process_tracking.os.getpid", return_value=4321), patch(
        "chemstack.core.utils.process_tracking.now_utc_iso",
        return_value="2026-03-22T00:00:00+00:00",
    ), patch("chemstack.core.utils.process_tracking.process_lock.current_process_start_ticks", return_value=None):
...
def test_active_run_lock_pid_covers_invalid_dead_reuse_and_logger_paths(tmp_path: Path) -> None:
    ...
    with patch("chemstack.core.utils.process_tracking.process_lock.parse_lock_info", return_value={"pid": "bad"}):
        assert process_tracking.active_run_lock_pid(reaction_dir) is None

    with patch("chemstack.core.utils.process_tracking.process_lock.parse_lock_info", return_value={"pid": 123}), patch(
        "chemstack.core.utils.process_tracking.process_lock.is_process_alive",
        return_value=False,
    ):
        assert process_tracking.active_run_lock_pid(reaction_dir) is None
    ...
    on_pid_reuse = Mock()
    with patch(
        "chemstack.core.utils.process_tracking.process_lock.parse_lock_info",
        return_value={"pid": 456, "process_start_ticks": 111},
    ), patch("chemstack.core.utils.process_tracking.process_lock.is_process_alive", return_value=True), patch(
        "chemstack.core.utils.process_tracking.process_lock.process_start_ticks",
        return_value=None,
    ):
        assert (
    ...
    with patch("chemstack.core.utils.process_tracking.process_lock.parse_lock_info", return_value={"pid": 654}), patch(
        "chemstack.core.utils.process_tracking.process_lock.is_process_alive",
        return_value=True,
    ):
...
def test_read_pid_file_covers_missing_invalid_dead_unlink_failure_and_live_pid(tmp_path: Path) -> None:
    ...
    with patch("chemstack.core.utils.process_tracking.process_lock.is_process_alive", return_value=False), patch(
        "pathlib.Path.unlink",
        autospec=True,
    ...
    with patch("chemstack.core.utils.process_tracking.process_lock.is_process_alive", return_value=True):
    ...
    with patch("chemstack.core.utils.process_tracking.process_lock.is_process_alive", return_value=True), patch(
        "chemstack.core.utils.process_tracking.process_lock.process_start_ticks",
        return_value=111,
    ):
