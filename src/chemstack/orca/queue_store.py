...
def _normalized_raw(raw: dict[str, Any]) -> dict[str, Any]:
    ...
    reaction_dir = _normalize_text(metadata.get("reaction_dir"))
    force = _normalize_bool(metadata.get("force", False))
    ...
    queue_id = _normalize_text(normalized.get("queue_id"))
    task_id = _normalize_text(normalized.get("task_id")) or queue_id
    if task_id:
    ...
    normalized["status"] = (
        _normalize_text(normalized.get("status")).lower() or QueueStatus.PENDING.value
    )
    normalized["started_at"] = _normalize_optional_text(normalized.get("started_at")) or ""
    normalized["finished_at"] = _normalize_optional_text(normalized.get("finished_at")) or ""
    normalized["error"] = _normalize_optional_text(normalized.get("error")) or ""
    normalized["cancel_requested"] = _normalize_bool(normalized.get("cancel_requested", False))
...
def queue_entry_run_id(entry: QueueEntry) -> str | None:
    return _normalize_optional_text(entry.metadata.get("run_id"))


def queue_entry_id(entry: QueueEntry) -> str:
    return _normalize_text(entry.queue_id)


def queue_entry_task_id(entry: QueueEntry) -> str | None:
    task_id = _normalize_text(entry.task_id)
...
def queue_entry_reaction_dir(entry: QueueEntry) -> str:
    return _normalize_text(entry.metadata.get("reaction_dir"))


def queue_entry_force(entry: QueueEntry) -> bool:
    return _normalize_bool(entry.metadata.get("force", False))
...
def queue_entry_app_name(entry: QueueEntry) -> str:
    return _normalize_text(entry.app_name) or QUEUE_APP_NAME
