...
def format_monitor_message(report: ScanReport, *, now: datetime | None = None) -> str:
    if now is None:
        ...
        current_time = now
    header = f"\u2699\ufe0f <b>chemstack monitor</b>  <code>{current_time.strftime('%Y-%m-%d %H:%M %Z')}</code>"
    divider = "\u2500" * 28
    scope = (
        "\U0001f50d <b>Scope</b>\n"
...
def format_run_started_event(event: RunStartedNotification) -> str:
    ...
    title = "ChemStack ORCA Resumed" if event.get("resumed") else "ChemStack ORCA Started"
    lines = [
...
def format_run_finished_event(event: RunFinishedNotification) -> str:
    ...
    reaction_dir = Path(event["reaction_dir"])
    status = str(event["status"]).strip().lower()
    title = "ChemStack ORCA Completed" if status == "completed" else "ChemStack ORCA Failed"
