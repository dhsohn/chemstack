...
def sibling_app_command(
    *,
    ...
    return (
        [sys.executable, "-m", module_name, "--config", config_path, *tail_argv],
        str(root_path),
        env,
    )
...
def _load_sibling_config(config_path: str) -> tuple[Path, dict[str, Any]]:
    import yaml  # type: ignore[import-untyped]
...
def sibling_runtime_paths(config_path: str, *, engine: str | None = None) -> dict[str, Path]:
    path, raw = _load_sibling_config(config_path)
    if engine in {"xtb", "crest"}:
