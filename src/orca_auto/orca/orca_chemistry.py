"""Small chemistry helpers used by ORCA parser internals."""
...
def build_formula(elements: list[str]) -> str:
    """Build a Hill system chemical formula from a list of element symbols."""
    counts = Counter(elements)
    if not counts:
        return ""

    # Hill system: C first, H next, then remaining in atomic number order.
    parts: list[str] = []
    for sym in ("C", "H"):
        if sym in counts:
