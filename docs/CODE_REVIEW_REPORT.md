...
| [notifier_runtime.py:129-130](core/notifier_runtime.py#L129-L130) | dedup state 저장 실패 무시 |
...
### 3.3 [MEDIUM] `_overflow_drop`의 스레드 안전성

**파일**: [notifier_runtime.py:31-63](core/notifier_runtime.py#L31-L63)

큐를 완전히 drain → 메모리에서 처리 → 다시 enqueue하는 패턴입니다. drain과 re-enqueue 사이에 다른 스레드가 `put_nowait`을 호출하면, re-enqueue 시 `queue.Full`로 이벤트가 유실될 수 있습니다.

**권장**:
- `_overflow_drop`에서 drain 전 큐를 사용하는 다른 경로를 고려하거나
- Lock을 사용하여 drain-process-enqueue를 atomic하게 처리
- 최소한 re-enqueue 실패 시 로깅 강화
...
| 13 | `_overflow_drop` 스레드 안전성 | 3.3 | 높음 |
...
- **Notification dedup 시스템**: 중복 알림 방지 및 비동기 전송 큐가 잘 설계됨
