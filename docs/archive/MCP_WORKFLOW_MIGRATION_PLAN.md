...
> Historical note: this document captures the migration plan that led to the current monorepo layout. The live implementation now sits under `src/chemstack/orca`, and new code should import `chemstack.orca.*` rather than `core.*` or `orca_auto.*`. For the active package map, see [../DEVELOPMENT.md](../DEVELOPMENT.md).
